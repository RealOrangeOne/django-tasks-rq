...
from django_tasks_rq.task import (
    DEFAULT_PRIORITY,
    MAX_PRIORITY,
    ResultStatus,
    Task,
...
from django_tasks_rq.task import TaskResult as BaseTaskResult
...
class Job(BaseJob):
    def perform(self) -> Any:
        ...
        self.meta.setdefault("_django_tasks_rq_worker_ids", []).append(self.worker_name)
    ...
    def check(self, **kwargs: Any) -> Iterable[messages.CheckMessage]:
        ...
        if not apps.is_installed("django_rq"):
            yield messages.Error(
                f"{backend_name} configured as django_tasks_rq backend, but django_rq app not installed",
